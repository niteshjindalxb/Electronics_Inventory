    ...
    def PrettyDumpClassic1K(self, key, uid, pretty=True):
        ...
        for i in range(0, 64, 4):
            ...
            if status == self.MI_OK:
                ...
                roll_no.append(self.Read(i, prettyPrint=pretty))
                roll_no.append(self.Read(i + 1, prettyPrint=pretty))
                roll_no.append(self.Read(i + 2, prettyPrint=pretty))
                roll_no.append(self.Read(i + 3, prettyPrint=pretty))
            else:
                print("Authentication error")
                raise errors.AuthenticationException
        bakwas=(str(roll_no[9][6]%16)+str(roll_no[9][7]%16)+str(roll_no[9][8]%16)+str(roll_no[9][9]%16)+str(roll_no[8][6]%16)+str(roll_no[8][7]%16)+str(roll_no[8][8]%16)+str(roll_no[8][9]%16)+str(roll_no[6][6]%16))
        print(bakwas)
